...
class ModelSaleDpdfrance extends Model {
	...
	public function editOrder($order_id, $data) {
		...
		$order_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order` WHERE order_status_id NOT IN (0,3,7) AND order_id = '" . (int)$order_id . "'");
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id, CONCAT(o.shipping_firstname, ' ', o.shipping_lastname) AS customer, o.shipping_country_id, SUBSTR(o.shipping_method,24) AS relais_address, CONCAT(o.shipping_address_1, ' ', o.shipping_postcode, ' ' , o.shipping_city) AS address, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.total, o.shipping_code, o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o";

		if (isset($data['filter_order_status_id']) && !is_null($data['filter_order_status_id'])) {
			$sql .= " WHERE o.shipping_code LIKE 'dpdfr%' AND o.order_status_id = '" . (int)$data['filter_order_status_id'] . "'";
		} else {
			$sql .= " WHERE o.shipping_code LIKE 'dpdfr%' AND o.order_status_id NOT IN (0,3,7)";
		}
		...
		if (!empty($data['filter_address'])) {
			$sql .= " AND CONCAT(o.shipping_address_1, ' ', o.shipping_postcode, ' ' ,o.shipping_city, SUBSTR(o.shipping_method,24)) LIKE '%" . $this->db->escape($data['filter_address']) . "%'";
		}
	...
	public function getTotalOrders($data = array()) {
		...
		if (isset($data['filter_order_status_id']) && !is_null($data['filter_order_status_id'])) {
			...
			$sql .= " WHERE order_status_id NOT IN (0,3,7)";
		}
		...
		if (!empty($data['filter_customer'])) {
			$sql .= " AND CONCAT(shipping_firstname, ' ', shipping_lastname) LIKE '%" . $this->db->escape($data['filter_customer']) . "%'";
		}

		if (!empty($data['filter_address'])) {
			$sql .= " AND CONCAT(shipping_address_1, ' ', shipping_postcode, ' ', shipping_city) LIKE '%" . $this->db->escape($data['filter_address']) . "%'";
		}
		
		if (!empty($data['filter_shipping_code'])) {
			$sql .= " AND shipping_code =" . $this->db->escape($data['filter_shippingcode']) . "'";
		}
	...
	public function getTotalOrdersByOrderStatusId($order_status_id) {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id = '" . (int)$order_status_id . "' AND order_status_id NOT IN (0,3,7)");

		return $query->row['total'];
	}

	public function getTotalOrdersByLanguageId($language_id) {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE language_id = '" . (int)$language_id . "' AND order_status_id NOT IN (0,3,7)");

		return $query->row['total'];
	}

	public function getTotalOrdersByCurrencyId($currency_id) {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE currency_id = '" . (int)$currency_id . "' AND order_status_id NOT IN (0,3,7)");

		return $query->row['total'];
	}
	
	public function getTotalSales() {
      	$query = $this->db->query("SELECT SUM(total) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id NOT IN (0,3,7)");

		return $query->row['total'];
	}

	public function getTotalSalesByYear($year) {
      	$query = $this->db->query("SELECT SUM(total) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id NOT IN (0,3,7) AND YEAR(date_added) = '" . (int)$year . "'");
